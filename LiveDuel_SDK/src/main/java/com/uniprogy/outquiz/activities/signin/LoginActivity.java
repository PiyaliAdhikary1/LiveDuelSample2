...
public class LoginActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(intent.hasExtra("backgroundImageID") && intent.hasExtra("iconID")  && intent.hasExtra("IPAdress")){
            /*Bundle extras = getIntent().getExtras();
            byte[] b = extras.getByteArray("drawable");

            Bitmap bmp = BitmapFactory.decodeByteArray(b, 0, b.length);*/
            ImageView image = (ImageView) findViewById(R.id.bcg);
            ImageView iconImage = (ImageView) findViewById(R.id.logoImageView);

            preferences=getApplicationContext().getSharedPreferences("IpAddress", MODE_PRIVATE);
            SharedPreferences.Editor editor=preferences.edit();
            editor.putString("IpAddress", getIntent().getStringExtra("IPAdress"));
            editor.commit();



            try{
                String backgroundImage=getResources().getResourceName(getIntent().getIntExtra("backgroundImageID",0));
                String icon=getResources().getResourceName(getIntent().getIntExtra("iconID",0));
                if(backgroundImage.length()>0 && icon.length()>0){

                    image.setImageResource(getIntent().getIntExtra("backgroundImageID",0));
                    iconImage.setImageResource(getIntent().getIntExtra("iconID",0));
                    HomeActivity.backgroundImageID=getIntent().getIntExtra("backgroundImageID",0);
                    HomeActivity.iconID=getIntent().getIntExtra("iconID",0);
                }else {
                    Toast.makeText(this, "Please Enter a Valid Background or Icon Image", Toast.LENGTH_SHORT).show();
                }
