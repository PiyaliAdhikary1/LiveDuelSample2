...
public class ShowActivity extends BaseActivity {
    ...
    private class CacheMedia extends Media {
    ...
    String socketData, streamingData;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        socketData = i.getStringExtra("socket");
        streamingData = i.getStringExtra("streaming");
    ...
    {
        try {
            JSONObject streamingJson = new JSONObject(streamingData);
            String url = streamingJson.getString("scheme")+"://"
                    + streamingJson.getString("host") + ":"
                    + streamingJson.getString("port") + "/"
                    + streamingJson.getString("application") + "/"
                    + streamingJson.getString("key");

            int networkCaching = 250;
            ...
            Media media = new CacheMedia(mLibVLC, Uri.parse(url));
            media.setHWDecoderEnabled(true, false);
            media.addOption(":network-caching=" + String.valueOf(networkCaching));
    ...
    {
        try {
            JSONObject socketJson = new JSONObject(socketData);
            ...
            socket = IO.socket("http://"+socketJson.getString("host")+":"+socketJson.getString("port"), opts);
        } catch (Exception e) {
            e.printStackTrace();
        }
